...
RSpec.describe Api::EventsController, type: :request do
  ...
  describe 'POST /api/events' do
    it 'creates a new event for the authenticated user' do
      ...
      sign_in user
      post '/api/events', params: { event: attributes_for(:event) }
    ...
    it 'returns unprocessable_entity for invalid event data' do
      ...
      sign_in user
      post '/api/events', params: { event: { name: '' } }
  ...
  describe 'PUT /api/events/:id' do
    it 'updates the details of a specific event for the authenticated user' do
      ...
      sign_in user
      put "/api/events/#{event.id}", params: { event: { name: 'Updated Name' } }
    ...
    it 'returns unauthorized for unauthenticated user' do
      ...
      put "/api/events/#{event.id}", params: { event: { name: 'Updated Event Name' } }
  ...
  describe 'POST /api/events/add_user_to_events' do
    it 'adds the authenticated user to a specific event' do
      user = create(:user)
      event = create(:event)
  
      sign_in user
      post '/api/events/add_user_to_events', params: { event_id: event.id }
    ...
    it 'returns unprocessable_entity if the event is not found' do
      ...
      sign_in user
      post '/api/events/add_user_to_events', params: { event_id: 999 }
  ...
  describe 'GET /api/events/get_events' do
    ...
    it 'returns "No events found" if all events are joined by the authenticated user' do
      user = create(:user)
      event = create(:event)
  ...
  describe 'GET /api/events/joined_events' do
    ...
    it 'returns "No events found" if the authenticated user has not joined any events' do
      user = create(:user)
